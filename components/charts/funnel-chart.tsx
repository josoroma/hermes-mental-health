...
export function FunnelChartView({ measure, answers, sampleLabel }: FunnelChartViewProps) {
  const scores = extractFieldScores(measure, answers, COLORS);
  const data = scores.map((s, i) => ({
    name: s.label,
    value: Math.max(s.score, 0.5),
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">Support Needs Funnel</CardTitle>
        {sampleLabel && <p className="text-sm text-muted-foreground">{sampleLabel}</p>}
      </CardHeader>
      <CardContent>
        <ChartContainer config={{}} className="mx-auto" initialDimension={{ width: 380, height: 320 }}>
          <FunnelChart>
