...
(async () => {
  ...
  try {
    ...
    // 8b. Result Detail — scrolled to answers section
    // MUST exist: narration says "answers section breaks down each item response"
    await page.evaluate(() => window.scrollTo(0, 600));
    await page.waitForTimeout(500);
    await shot('result-dep-followup-scrolled.png');
    await page.evaluate(() => window.scrollTo(0, 0));

    // 9. Result Detail — edit mode (click last "Edit" button = Responses section)
    ...
    // 11. Session Edit — MDX editor
    // MUST exist: narration says "Edit link opens the MDX editor"
    ...
    // 12. Agent Chat — MUST use /agent not /patients/<id>/agent-chat (404!)
    await shot('agent-chat-patient.png', `${BASE}/agent?sessions&patientId=${PID}`);

    // 13. Assessment Editor — Metadata tab (default view)
    await shot('editor.png', `${BASE}/editor/level1-adult`);

    // 13b. Assessment Editor — Fields tab
    // MUST exist: narration says "four tabs"
    ...
    // 13c. Assessment Editor — Scoring tab (optional, for future use)
